...
public class AuthorController {
    ...
    public List<AuthorDto> listAuthors() {
        ...
        return authorList.stream().map(authorMapper::mapTo).collect(Collectors.toList());
    }

    @GetMapping(path = "/authors/{id}")
    public ResponseEntity<AuthorDto> getAuthor(@PathVariable("id") Long id) {
        Optional<AuthorEntity> retrievedAuthor = authorService.findOne(id);
        return retrievedAuthor.map(authorEntity -> {
    ...
    public ResponseEntity<AuthorDto> fullUpdateAuthor(@PathVariable("id") Long id, @RequestBody AuthorDto authorDto) {
        if (!authorService.isExistent(id)) {
